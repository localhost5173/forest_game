...
@onready var colossal_runestone = preload("res://scenes/bluer_runestone.tscn")
...
func spawn_altars():
	var density = 0.0015  # Rare altars
	var step = 4.0
	var altar_positions = []
	...
	for i in range(0, int(chunk_size), int(step)):
		for j in range(0, int(chunk_size), int(step)):
			...
			var world_y = noise.get_noise_3d(world_x, 0, world_z) * 25
	...
	for i in range(0, int(chunk_size), int(step)):
		for j in range(0, int(chunk_size), int(step)):
			...
			for altar_pos in altar_positions:
				if Vector2(world_x - x, world_z - z).distance_to(altar_pos) < 20.0:
					too_close = true
					break
			if too_close:
				continue
...
func spawn_runestones(altar_positions, tree_positions):
	var base_density = 0.001
	var magma_density = base_density
	var bluer_density = base_density * 10  # 2x more frequent than original magma density
	var colossal_density = base_density / 2  # 1/50th of original magma density
	var step = 2.0
	...
	for i in range(0, int(chunk_size), int(step)):
		for j in range(0, int(chunk_size), int(step)):
			...
			# Check if too close to other objects (2m radius)
			var too_close = false
			for pos in tree_positions + altar_positions + runestone_positions:
			...
			# Check for Colossal Runestone (rarest first)
			if randf() < colossal_density:
				instance = colossal_runestone.instantiate()
			
			# Check for Bluer Runestone (water level only)
			elif world_y <= WATER_LEVEL and randf() < bluer_density:
				instance = bluer_runestone.instantiate()
				world_y = WATER_LEVEL  # Snap to water surface
			
			# Check for Magma Runestone
			elif randf() < magma_density:
